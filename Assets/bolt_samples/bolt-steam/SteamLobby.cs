...
public class NetworkCallbacks : Bolt.GlobalEventListener
{
    ...
    {
        var token = (SteamToken)c.ConnectToken;
        ...
        foreach (var m in activeLobby.AllMembers)
        {
            ...
            {
                c.UserData = "CLIENT:" + SteamFriends.GetFriendPersonaName(m) + " " + c.RemoteEndPoint.Port;
    ...
    {
        if (SteamManager.Initialized && SteamHub.LobbyActive != null)
        {
...
public class SteamLobby : MonoBehaviour
{
    ...
    {
        var id = SteamHub.LobbyActive.ServerId;
        return id;
    }
    ...
    private void steamPeer_Host_Failed()
    {
        Debug.Log("HOST FAILED CALLBACK");
    }

    private void steamPeer_Host_Disconnected()
    {
        Debug.Log("HOST DISCONNECTED CALLBACK");
    }

    private void steamPeer_Host_Connected(CSteamID lobby)
    {
